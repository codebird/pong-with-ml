...
class Player(pygame.sprite.Sprite):
    ...
    def update_ml(self, ball):
        # This gets the position of the axis on the game controller
        # It returns a number between -1.0 and +1.0
        vertical_axis_pos = clf.predict([[ball.x, ball.y, ball.direction, ball.speed]])

        # Move x according to the axis. We multiply by 15 to speed up the movement.
        self.rect.y = int(vertical_axis_pos)
        # Make sure we don't push the player paddle off the right side of the screen
...
while not exit_program:
    ...
    if not done:
        ...
        player1.update_ml(ball)
        player2.update_where_ball_is(ball)
        # Comment the above line and uncomment this to play against.
        #player2.update()
        ball.update()
